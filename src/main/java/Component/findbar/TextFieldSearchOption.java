...
public class TextFieldSearchOption extends JTextField {
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            JFrame frame = new JFrame("Search Field Test");
            frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
            frame.setSize(400, 200);

            // Tạo TextFieldSearchOption
            TextFieldSearchOption searchField = new TextFieldSearchOption();

            // Thêm các SearchOption
            searchField.addOption(new SearchOption("Tìm mã sv", new ImageIcon(TextFieldSearchOption.class.getResource("/dashboard/icon/num.png"))));
            searchField.addOption(new SearchOption("Tìm tên", new ImageIcon(TextFieldSearchOption.class.getResource("/dashboard/icon/user.png"))));
            searchField.addOption(new SearchOption("Bing", new ImageIcon(TextFieldSearchOption.class.getResource("/dashboard/icon/class.png"))));
            searchField.addOption(new SearchOption("Tìm email", new ImageIcon(TextFieldSearchOption.class.getResource("/dashboard/icon/email.png"))));
            searchField.addOption(new SearchOption("Yahoo", new ImageIcon("path_to_yahoo_icon.png")));

            // Lắng nghe sự kiện khi tùy chọn được chọn
            searchField.addEventOptionSelected((option, index) -> {
                System.out.println("Selected Option: " + option.getName() + " (Index: " + index + ")");
            });
